...
def check_password():
    ...
    st.markdown("## 🔒 Document Enhancer Login")
    ...
    with col2:
        ...
        st.markdown("---")
        st.info("💡 **For administrators**: Set your custom password by adding `APP_PASSWORD=your_password` to your `.env` file")
        
    return False

def cleanup_old_documents(docs_dir, keep_current_id=None):
    """
    Clean up old document folders, keeping only the current one.
    
    Args:
        docs_dir (Path): The documents directory containing document folders
        keep_current_id (str): ID of the current document being processed (don't delete this)
    ...
    try:
        ...
        # Get all document folders (folders that look like document IDs)
        deleted_count = 0
        for item in docs_dir.iterdir():
            if (item.is_dir() and 
                len(item.name) == 8 and 
                item.name != keep_current_id):
                try:
                    shutil.rmtree(item)
                    deleted_count += 1
                    print(f"🗑️ Deleted old document folder: {item.name}")
...
def start_document_processing(uploaded_file, process_images=True, batch_size=1):
    """Start document processing with selected processor and cleanup old files"""
    try:
        # 🧹 CLEANUP: Remove old documents before processing new one
        with st.spinner("🧹 Cleaning up old documents..."):
            deleted_count = cleanup_old_documents(DOCS_DIR)
            if deleted_count > 0:
                st.success(f"🧹 Cleaned up {deleted_count} old document folders")
...
def main():
    ...
    # Show project info with cleanup info
    st.info(f"📁 Documents are stored in: {PROJECT_DIR}")
    st.info("🧹 **Auto-cleanup enabled**: Old documents are automatically removed when you upload a new file")
    ...
    with col1:
        ...
        if st.session_state.processing_status not in ["processing_in_progress"]:
            ...
            # Show cleanup warning if file is uploaded
            if uploaded_file:
                st.warning("⚠️ **Note**: Processing this document will remove any previously processed documents to save storage space.")
